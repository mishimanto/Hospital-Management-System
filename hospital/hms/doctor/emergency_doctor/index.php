...
if (isset($_POST['submit'])) {
    ...
    if ($num) {
        ...
        $_SESSION['id']     = $num['id'];
        $_SESSION['role']   = 'doctor'; // optional role flag
        $uid                = $num['id'];
    ...
    // If not found, check emergency doctors table
    ...
    if ($em) {
        ...
        $_SESSION['name']   = $em['name']; // Optional: store name
        $uid                = $em['id'];
        $uip                = $_SERVER['REMOTE_ADDR'];
        $status             = 1;

        // Optional: separate log table or reuse same
        mysqli_query($con, "INSERT INTO doctorslog(uid, username, userip, status) 
                            VALUES('$uid', '$uname', '$uip', '$status')");
        header("location:dashboard.php");
        exit();
...
<body>
    ...
    <div class="login-container pulse">
        <div class="logo">
            <a href="../../index.php">
                <h2><i class="fas fa-heartbeat"></i>Emergency Doctor Portal</h2>
            </a>
        </div>
